...
static void set_view_type(ShellViewType viewtype, gboolean reload)
{
	...
	gtk_widget_get_allocation(GTK_WIDGET(shell->hpaned), alloc);
	...
	GtkAllocation* allox = g_new(GtkAllocation, 1);
	gtk_widget_get_allocation(GTK_WIDGET(shell->hpaned), allox);
	gtk_paned_set_position(GTK_PANED(shell->vpaned),
			       allox->height -
...
module_selected_show_info(ShellModuleEntry * entry, gboolean reload)
{
    ...
    /* */
    gdk_window_freeze_updates(gtk_widget_get_window(shell->info->view));
    ...
    gdk_window_thaw_updates(gtk_widget_get_window(shell->info->view));
